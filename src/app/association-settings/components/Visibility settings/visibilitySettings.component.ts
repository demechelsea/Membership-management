...
export class VisibilitySettingsComponent extends BaseComponent implements OnInit {
  ...
  getRoleAndPermission(roleAndPermissionid: number) {
    this.roleAndPermissiontService.getRoleAndPermission(roleAndPermissionid)
  ...
  onSelectedMembersOption(option: any) {
    this.visibilitySettingForm.controls['member'].setValue(`${option.userDetail.firstName} ${option.userDetail.givenName} ${option.userDetail.parentName}`);
  ...
  applyRoleAndPermission() {
    this.roleAndPermissiontService.updateRoleAndPermission(this.selectedRoleAndPermission)
      .pipe(takeUntil(this.ngUnsubscribe$))
      .subscribe(response => {
      });
  }
